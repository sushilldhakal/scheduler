...
Each preset calls `createSchedulerConfig` with sensible defaults for that domain:
...
// Workforce roster
const config = createSchedulerConfig({ preset: 'roster', { initialScrollToNow: true } })
