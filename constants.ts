import type { RoleColor, Settings } from './types'

// ─── Layout Constants ─────────────────────────────────────────────────────────
export const HOUR_W: number = 88
export const SNAP: number = 0.5
export const SIDEBAR_W: number = 190
export const SHIFT_H: number = 42
export const ROLE_HDR: number = 38
export const HOUR_HDR_H: number = 44
...
export const MONTHS_SHORT: readonly string[] = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
...
export const rc = (idx: number): RoleColor => ROLE_COLORS[idx % ROLE_COLORS.length]
...
export const getFirst = (y: number, m: number): number => { const d = new Date(y, m, 1).getDay(); return d === 0 ? 6 : d - 1 }

export function fmt12(h: number): string {
  const w = Math.floor(h), m = Math.round((h - w) * 60)
  if (w === 0 && !m) return "12am"
...
export function get3Weeks(date: Date): Date[][] {
  ...
  const p = new Date(c[0]); p.setDate(p.getDate() - 7)
  const n = new Date(c[0]); n.setDate(n.getDate() + 7)
