    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "16px 12px" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 16 }}>
        {MONTHS.map((mName, m) => {
          ...
          const ms    = shifts.filter(s => s.date.getFullYear() === year && s.date.getMonth() === m)
          ...
          return (
            <div
              ...
              style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", padding: "12px", cursor: "pointer", transition: "box-shadow 0.15s,transform 0.15s", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}
            ...
            >
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", marginBottom: 2 }}>
                {"MTWTFSS".split("").map((c, i) => (
                  <div key={i} style={{ textAlign: "center", fontSize: 8, color: "#d1d5db", fontWeight: 700 }}>{c}</div>
                ))}
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 1 }}>
                {cells.map((d, i) => {
                  ...
                  return (
                    <div
                      key={d}
                      style={{ textAlign: "center", fontSize: 9, fontWeight: tod ? 700 : 400, color: tod ? "#fff" : has ? "#3b82f6" : "#9ca3af", background: tod ? "#3b82f6" : has ? "#dbeafe" : "transparent", borderRadius: 2, lineHeight: "16px" }}
                    >
                      {d}
                    </div>
                  )
              ...
              <div style={{ marginTop: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <div style={{ width: 6, height: 6, borderRadius: "50%", background: ms.length > 0 ? "#3b82f6" : "#e5e7eb" }} />
