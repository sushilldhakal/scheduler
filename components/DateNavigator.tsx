...
import { ICalendar, IChevL, IChevR } from "../icons"
...
export function DateNavigator({ view, currentDate, onDateChange, onNavigate, shifts }: DateNavigatorProps): JSX.Element {
  ...
  return (
    <div className="flex items-center gap-3">
      {/* Month / Year label + event count */}
      <div className="flex items-center gap-2">
        <span className="text-lg font-semibold text-foreground">
          {MONTHS[currentDate.getMonth()]} {currentDate.getFullYear()}
        </span>
        <div className="inline-flex items-center px-2 py-1 bg-transparent border border-border rounded-full text-xs font-medium text-muted-foreground">
          {eventCount} events
        </div>
      </div>
      ...
      {/* Navigation controls */}
      <div className="flex items-center gap-2">
        <Button onClick={handlePrevious} variant="outline" size="icon" className="h-7 w-7">
          <IChevL size={16} />
        </Button>
        ...
        <Button onClick={handleNext} variant="outline" size="icon" className="h-7 w-7">
          <IChevR size={16} />
        </Button>
      </div>
    </div>
