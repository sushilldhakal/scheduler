...
export function GridViewSidebar({
  ...
  // Row heights from virtualizer so sidebar rows match grid rows exactly
  ...
  // Shifts visible in the current date window
  ...
  return (
    <>
      {/* Sticky header — aligns with grid date/hour header */}
      <div style={{
        position: "sticky",
        top: 0,
        zIndex: 5,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
          {(["name", "hours", "scheduled"] as const).map((col) => {
            ...
            return (
              <button key={col} type="button" onClick={() => toggleSort(col)} style={{
      ...
      {/* Rows — normal flow, category headers use position:sticky top:HOUR_HDR_H */}
      {flatRows.map((row) => {
            ...
            <div key={row.key} style={{
              ...
              <div style={{ height: ROLE_HDR, display: "flex", alignItems: "center", paddingLeft: 14, paddingRight: 8, gap: 6 }}>
                {slots.resourceHeader ? (
                  slots.resourceHeader({ resource: cat, scheduledCount: scheduled, isCollapsed: collapsed.has(cat.id), onToggleCollapse: () => toggleCollapse(cat.id) })
                ) : (
                  <>
                    <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 2 }}>
                      ...
                      <span style={{ fontSize: 10, color: "var(--muted-foreground)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", lineHeight: 1.2 }}>
                        {staffCount} staff{scheduled > 0 ? ` · ${scheduled} shift${scheduled !== 1 ? "s" : ""}` : ""}{totalHours > 0 ? ` · ${totalHours.toFixed(1)}h` : ""}
                      </span>
                    </div>
                    <button type="button" onClick={() => toggleCollapse(cat.id)} aria-label={collapsed.has(cat.id) ? "Expand" : "Collapse"} style={{
                      ...
                      transform: collapsed.has(cat.id) ? "rotate(-90deg)" : "none", transition: "transform 0.2s", flexShrink: 0, borderRadius: 4,
                    }}>
                    ...
                    <button type="button" onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                      ...
                      border: `1px solid ${c.border}`, borderRadius: 5, padding: "3px 7px", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0,
                    }}>
                      {labels.staff}
                    </button>
                  </>
              ...
              <div style={{ padding: "0 14px 5px", flexShrink: 0 }}>
                <div style={{ height: 4, borderRadius: 2, background: "var(--border)", overflow: "hidden" }}>
                  <div style={{ height: "100%", borderRadius: 2, background: isOverCapacity ? "var(--destructive)" : c.bg, width: `${hoursPercent}%`, transition: "width 300ms ease" }} />
                </div>
              </div>
            </div>
          )
        ...
        return (
          <div key={row.key} style={{
            ...
            display: "flex", alignItems: "center", paddingLeft: 14, paddingRight: 8, gap: 8,
            overflow: "hidden", transition: "background 80ms ease",
          }}>
            <div style={{
              width: 32, height: 32, borderRadius: "50%", background: `${c.bg}20`,
              border: `1.5px solid ${c.bg}40`, display: "flex", alignItems: "center", justifyContent: "center",
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "var(--foreground)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{emp!.name}</div>
              <div style={{ fontSize: 10, color: "var(--muted-foreground)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{empShifts.length} shift{empShifts.length !== 1 ? "s" : ""}</div>
            </div>
            {empHours > 0 && (
              <div style={{
      ...
      {/* Resize handle */}
      <div style={{ position: "absolute", top: 0, right: -4, width: 8, height: "100%", cursor: "col-resize", zIndex: 30 }}
        onPointerDown={(e) => {
          e.preventDefault()
      ...
      {/* Collapse toggle */}
      <div style={{ position: "fixed", top: "50%", left: sidebarCollapsed ? 2 : sidebarWidth - 1, transform: "translateY(-50%)", zIndex: 50, transition: "left 150ms ease" }}>
        <button onClick={toggleSidebar} style={{
          width: 16, height: 28, borderRadius: "0 4px 4px 0", background: "var(--background)",
